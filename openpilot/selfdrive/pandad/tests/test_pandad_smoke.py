...
def test_pandad_hardware_methods_exist():
  ...
  assert methods, "no HARDWARE calls found in pandad.py"

  missing = [m for m in sorted(methods) if not callable(getattr(HARDWARE, m, None))]
  assert not missing, f"HARDWARE is missing methods called in pandad.py: {missing}"


def test_pandad_multipanda_smoke():
  ...
  with patch.object(pandad, 'Panda') as mock_panda_class, \
    ...
    assert result.get('exited', False), \
