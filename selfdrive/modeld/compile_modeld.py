...
def compile_modeld(nv12: NV12Frame, model_w, model_h, prepare_only, frame_skip,
  ...
  def random_inputs_run_fn(fn, seed, test_val=None, test_buffers=None, expect_match=True):
    ...
    for i in range(N_RUNS):
      frame = Tensor.randint(nv12.size, low=0, high=256, device=WARP_DEV, dtype='uint8').realize()
      big_frame = Tensor.randint(nv12.size, low=0, high=256, device=WARP_DEV, dtype='uint8').realize()
      for v in npy.values():
